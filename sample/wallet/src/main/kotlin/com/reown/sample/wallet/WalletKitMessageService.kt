...
class WalletKitMessageService {
    private val logger: Logger by lazy { wcKoinApp.koin.get(named(AndroidCommonDITags.LOGGER)) }

    fun onMessage(message: Core.Model.Message, originalMessage: RemoteMessage) {
    ...
    fun registeringFailed(token: String, throwable: Throwable) {
        logger.log("Registering New Token Failed:\t$token")
    }

    fun onDefaultBehavior(message: RemoteMessage) {
        logger.log("onDefaultBehavior: ${message.to}")
    }

    fun onError(throwable: Throwable, defaultMessage: RemoteMessage) {
